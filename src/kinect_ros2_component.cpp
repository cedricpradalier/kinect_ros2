...
namespace kinect_ros2
{
		...
		: Node("kinect_ros2", options)
	{
		...
		if (ret < 0) {
			...
			RCLCPP_INFO(get_logger(), "FREENECT - OPEN");
		}
		ret =
		...
		if (ret < 0) {
			...
			RCLCPP_INFO(get_logger(), "FREENECT - SET DEPTH");
		}

		ret = freenect_set_video_mode(fn_dev_, freenect_find_video_mode(FREENECT_RESOLUTION_MEDIUM, FREENECT_VIDEO_RGB));
		if (ret < 0) {
			freenect_shutdown(fn_ctx_);
			RCLCPP_ERROR(get_logger(), "FREENECT - ERROR SET VIDEO");
			rclcpp::shutdown();
		} else {
			RCLCPP_INFO(get_logger(), "FREENECT - SET VIDEO");
		}
		...
		if (ret < 0) {
			...
			RCLCPP_INFO(get_logger(), "FREENECT - START DEPTH");
		}
		...
		if (ret < 0) {
			...
			RCLCPP_INFO(get_logger(), "FREENECT - START RGB");
		}
	}
	...
	void KinectRosComponent::timer_callback()
	{
		...
		if (_depth_flag) {
			// RCLCPP_INFO(get_logger(), "depth image available");
		...
		if (_rgb_flag) {
			// RCLCPP_INFO(get_logger(), "rgb image available");
